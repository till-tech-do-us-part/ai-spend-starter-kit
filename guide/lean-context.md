# Lean context: seven habits for coding with AI
...
## 3. Design for cache reuse

Keep stable instructions and reference material at the start of a prompt or session, and put changing task details later. Reuse the same stable prefix instead of casually rewriting it. Check the token usage meter's cache-hit flag to confirm that the provider is actually reusing input rather than assuming a cache is effective.

## 4. Audit connected tools
